 ...
 *
 * @param array $args {
 *     @type string $name     Icon-Name (check|shield|clock|star|award|tool|users|heart|location|currency)
 *     @type string $size     Größe (sm|md|lg|xl) - Standard: md
 *     @type string $color    Tailwind-Farbklasse - Standard: currentColor
 *     @type array  $classes  Zusätzliche Klassen
 *     @type string $label    Barrierefreies Label (sr-only)
 * }
 */
...
$classes = implode(' ', array_merge(
    [$size_map[$args['size']] ?? $size_map['md']],
    is_array($args['classes']) ? $args['classes'] : [$args['classes']]
));

?>

<svg class="<?php echo esc_attr($classes); ?>" fill="none" stroke="<?php echo esc_attr($args['color']); ?>" viewBox="0 0 24 24" aria-hidden="true">
    <?php if ($args['label']): ?>
        <title><?php echo esc_html($args['label']); ?></title>
    <?php endif; ?>
    <?php echo $icon_map[$args['name']] ?? $icon_map['check']; ?>
</svg>
