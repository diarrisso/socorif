...
# Script de déploiement complet pour BEKA Theme
...
LOCAL_WP_PATH="${LOCAL_WP_PATH:-../../..}"  # Chemin vers la racine WordPress locale
...
check_requirements() {
    ...
    if ! ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "command -v wp" &> /dev/null; then
...
deploy_theme_files() {
    ...
    # Rsync
    eval "rsync -avz --delete -e 'ssh -p $SSH_PORT' $EXCLUDE_ARGS '$SCRIPT_DIR/' '$SSH_USER@$SSH_HOST:$SSH_PATH/'"
...
backup_remote_database() {
    ...
    ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "cd $REMOTE_WP_PATH && wp db export $DB_BACKUP_FILE"
...
import_database() {
    ...
    scp -P "$SSH_PORT" "$LOCAL_WP_ABS/$DB_EXPORT_FILE" "$SSH_USER@$SSH_HOST:$REMOTE_WP_PATH/"

    log "Import de la base de données sur le serveur..."

    # Import
    ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "cd $REMOTE_WP_PATH && wp db import $DB_EXPORT_FILE"
...
search_replace_urls() {
    ...
    ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "cd $REMOTE_WP_PATH && wp search-replace '$LOCAL_URL' '$REMOTE_URL' --all-tables --skip-columns=guid"
...
flush_cache() {
    ...
    ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "cd $REMOTE_WP_PATH && wp cache flush"
...
cleanup() {
    ...
    # Remote
    ssh -p "$SSH_PORT" "$SSH_USER@$SSH_HOST" "cd $REMOTE_WP_PATH && rm -f $DB_EXPORT_FILE"
...
sync_uploads() {
    if confirm "Voulez-vous synchroniser les fichiers uploads (media) ?"; then
        ...
        rsync -avz --delete -e "ssh -p $SSH_PORT" \
            "$LOCAL_WP_ABS/wp-content/uploads/" \
            "$SSH_USER@$SSH_HOST:$REMOTE_WP_PATH/wp-content/uploads/"
...
deployment_menu() {
    ...
    echo ""
    echo "  1) Tout (Code + Base de données + Uploads)"
